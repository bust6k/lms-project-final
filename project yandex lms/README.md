...
До конца законченные тесты отмечены //go: build unit. Также проект не является  хорошо масштабируемым. Это доказывает сложность написания тестов.  Видимо это тот самый момент когда нужно было использовать DI и clean architecture.
